            ...
            .then((response) => {
                statusEmbed
                    ...
                    .addFields(
                        ...
                        { name: 'Personas online', value: `${response.onlinePlayers} / ${response.maxPlayers}`, inline: true },
                    )
                ...
                if (response.onlinePlayers - response.samplePlayers.length == 0) {
                    ...
                    statusEmbed.addField('Jugadores', `${players}y ${response.onlinePlayers - response.samplePlayers.length} más`);
                }
            })
            .catch((error) => { // when server is off
                statusEmbed
                    .setColor('#FF4136')
                    .addFields(
